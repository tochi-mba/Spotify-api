...
from spotify_api.errors import SpotifyAuthError, SpotifyRateLimitError, SpotifyUnavailableError
...
class FakeSearchClient:
    ...
    async def search_track(self, item: LookupItem, *, market: str | None) -> Any:
...
async def test_a_found_track_is_mapped_onto_the_result(
    ...
    [result] = await resolver.resolve([LookupItem(name="Bohemian Rhapsody")])
...
async def test_no_match_is_reported_as_not_found(build_resolver: Any) -> None:
    ...
    [result] = await resolver.resolve([LookupItem(name="nothing here")])
...
async def test_one_failing_item_does_not_poison_the_batch(
    ...
    results = await resolver.resolve([LookupItem(name="good"), LookupItem(name="bad")])
...
async def test_every_known_upstream_failure_becomes_an_error_result(
    ...
    [result] = await resolver.resolve([LookupItem(name="x")])
...
async def test_concurrency_is_capped_at_the_configured_limit(build_resolver: Any) -> None:
    ...
    await resolver.resolve([LookupItem(name=f"track {n}") for n in range(12)])
...
async def test_the_request_market_wins_over_the_configured_default(
    ...
    await resolver.resolve([LookupItem(name="x")], market="GB")
    await resolver.resolve([LookupItem(name="x")])
